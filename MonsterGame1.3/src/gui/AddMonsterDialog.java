...
public class AddMonsterDialog extends JDialog implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == cancelButton) {
			...
			Object monsterType = typeCombo.getSelectedItem(); // Store the data values for the different fields
			
			Object monsterHitpoints = hitPointSpinner.getValue();
			
			String monsterAttack1 = attack1Field.getText();
			
			Object monsterAttack1hp = attack1Spinner.getValue();
			
			String monsterAttack2 = attack2Field.getText();
			
			Object monsterAttack2hp = attack2Spinner.getValue();
			
			Attack newAttack1 = new Attack(monsterAttack1, (int) monsterAttack1hp); // Store the attacks
			Attack newAttack2 = new Attack(monsterAttack2, (int) monsterAttack2hp);
			
			Attack[] attackArray = new Attack[] {newAttack1, newAttack2}; // Create the attack array and store respective attacks
			
			Monster monsterToAdd = Trainer.createMonster((String) monsterType, (int) monsterHitpoints, attackArray); // Create the monster
			
			trainer.addMonster(monsterToAdd); // Add monster to the trainer
			
			
			
			this.model.addElement(monsterToAdd); // Add to the defaultListModel of the trainer 
