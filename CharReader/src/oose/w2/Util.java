...
public class Util {
	...
	public void customCharReader(File file, int n) {
		...
		try {
			...
			throw e; // Added catch for RunTime exception
		} 	// This way we can differentiate the different exceptions thrown.
		catch(Exception e) {
	...
	public static String charReplace(String text, char oldchar, char newchar) {
		text = text.replace(oldchar, newchar); // Re-assign text so that 		
		return text; 					// replace does not create a new string
	}
}
